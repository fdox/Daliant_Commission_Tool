...
git rev-parse --is-inside-work-tree >/dev/null 2>&1 || {
  echo "❌ Run this from your Git repo root."
  exit 1
}
...
if [ -d "Views" ]; then
  ...
  VIEWS_DIR="Views"; mkdir -p "$VIEWS_DIR"
fi

############################################
# ProjectsHomeView.swift — @MainActor preview + Item(title:)
############################################
...
############################################
# SettingsView.swift — @MainActor preview helper
############################################
...
############################################
# If SignInView.swift exists, normalize its preview (remove return).
############################################
SIV="$VIEWS_DIR/SignInView.swift"
if [[ -f "$SIV" ]]; then
  # Remove 'return ' ONLY inside #Preview blocks, then ensure a simple one-liner preview exists.
  /usr/bin/python3 <<'PY'
import os, re
path = os.environ["SIV"]
...
# Ensure at least one basic preview exists (idempotent).
if "#Preview(" not in "\n".join(out):
    out.append("")
    out.append("#if DEBUG")
    out.append("#Preview(\"Sign In — Basic\") {")
    out.append("    SignInView()")
    out.append("}")
    out.append("#endif")
...
############################################
# As a safety net, strip any 'return ' inside #Preview blocks across all views.
############################################
...
p = r"""$f"""
s = open(p).read()
out=[]; inside=False; depth=0
for line in s.splitlines():
    ls=line.lstrip()
    if ls.startswith("#Preview"):
        inside=True; depth=0; out.append(line); continue
    if inside:
        depth += line.count("{") - line.count("}")
        if ls.startswith("return "):
            out.append(line[:len(line)-len(ls)] + ls[len("return "):])
        else:
            out.append(line)
        if depth <= 0:
            inside=False
...
if [[ -f "$SIV" ]]; then git add "$SIV"; fi
git add "$VIEWS_DIR"/*.swift

git commit -m "Phase 3 – Step 1e: Fix previews (remove return, single-expression bodies), mark preview helpers @MainActor, seed Items with title:."
git push origin main || echo "⚠️  git push failed (no remote/auth). Local commit created."

echo "✅ Applied: @MainActor for preview helpers, Item(title:) seeding, and return-stripping inside #Preview. Rebuild Canvas."
